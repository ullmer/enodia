...
########################################################
################# Enodia Scene Element #################

class enoSceneEl:

  overlayFn = "overlay1b" #default binding
  imgFn     = "person03f" #default binding

  #imgFn = "mri-fargate-08b2b"
  ...
  def buildVisuals(self): 
    self.actor1 = Actor(self.overlayFn, pos=self.currentPos)
    self.actor2 = Actor(self.imgFn,     pos=self.currentPos)

    self.actor2.angle = self.currentAngle
...
##################################################################
################# Enodia Scene Element Highlight #################

class enoSEhighlight:
  overlayFn   = "mri-fargate-09a-highlight2"
  actor      = None
  currentPos = (150, 150)
  ...
  def buildVisuals(self): 
    self.actor = Actor(self.overlayFn, pos=self.currentPos)
...
class enoSceneManager: 
  ...
  eseList = None #List of Enodia scene elements
  expectedFields = ['imgFn', 'overlayFn', 'pos', 'angle', 'key']
  ...
  def parseYamlFn(self, yamlFn):
    ...
    for row in self.yamlD: 
      for expectedField in self.expectedFields:
        if expectedField not in row: 
          print("Error processing YAML entry:", row)
          print("Required field %s not found; aborting" % expectedField)
          sys.exit(-1)

      ifn, ofn     = row['imgFn'], row['overlayFn']
      eangle, ekey = row['angle'], row['key']

      x, y = row['pos']
      epos = (x, y)       
       
      ese = enoSceneEl(imgFn=ifn, overlayFn=ofn, currentPos=epos, currentAngle=eangle, key=ekey)
      self.eseList.append(ese)

    eselen = len(self.eseList)

    print("%i scene elements loaded" % eselen)

  ################# draw #################
  
  def draw(self):                
    for ese in self.eseList: ese.draw()

  def on_key_down(self):         
    for ese in self.eseList: ese.on_key_down()

  def on_mouse_down(self, pos):  
    for ese in self.eseList: ese.on_mouse_down(pos)

  def on_mouse_move(self, pos):  
    for ese in self.eseList: ese.on_mouse_move(pos)

  def on_mouse_up(self):         
    for ese in self.eseList: ese.on_mouse_up()
